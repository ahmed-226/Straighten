...
def process_mask3d(ct_path,label_path,json_path,vertebrae_ids,output_folder,outputsize=(128,128,128)):
    ...
    # Clean up basename to match json keys (e.g. sub-verse012_ct -> sub-verse012)
    if basename.endswith('_ct'):
        basename = basename[:-3]
    ...
    for i,label in enumerate(vertebrae_ids):
        ...
        for entry in data:  # Skip the first entry which is 'direction'
            # Safe access: checking if 'label' exists in dictionary
...
def process_standard_verse_dataset(dataroot, output_folder, json_path, sample_size=None):
    """
    Process the standard Verse19 dataset structure.
    output naming will match vertebra_data.json keys (e.g., sub-verse012_17).
    """
    
    # Load vertebra data to know which vertebrae to process
    category_map = build_patient_vertebrae_map(json_path)
    
    # Flatten the map for easier lookup: patient_id -> list of vertebrae
    target_vertebrae = {}
    for cat, patients in category_map.items():
        for pid, v_ids in patients.items():
            target_vertebrae[pid] = v_ids

    # Define the sub-datasets to look for
    sub_datasets = ['dataset-verse19training', 'dataset-verse19validation', 'dataset-verse19test']
    
    for sub_dataset in sub_datasets:
        dataset_path = os.path.join(dataroot, sub_dataset)
        if not os.path.exists(dataset_path):
            print(f"Dataset folder not found: {dataset_path}, skipping...")
            continue
            
        print(f"Scanning {sub_dataset}...")
        
        rawdata_path = os.path.join(dataset_path, 'rawdata')
        derivatives_path = os.path.join(dataset_path, 'derivatives')
        
        if not os.path.exists(rawdata_path) or not os.path.exists(derivatives_path):
            print(f"Missing rawdata or derivatives in {sub_dataset}, skipping...")
            continue

        # Get list of subjects
        subjects = [d for d in os.listdir(rawdata_path) if os.path.isdir(os.path.join(rawdata_path, d))]
        subjects.sort()
        
        # Apply sampling if requested
        if sample_size is not None:
            subjects = subjects[:sample_size]
            print(f"Sampling first {sample_size} subjects from {sub_dataset}")

        for subject in subjects:
            # Construct paths
            subj_raw_path = os.path.join(rawdata_path, subject)
            subj_deriv_path = os.path.join(derivatives_path, subject)
            
            # Find CT file
            # Naming convention: sub-verse004_ct.nii or similar
            possible_cts = [f for f in os.listdir(subj_raw_path) if f.endswith('.nii') or f.endswith('.nii.gz')]
            possible_cts = [f for f in possible_cts if 'ct' in f.lower() or 'iso' in f.lower()] # simple filter
            
            if not possible_cts:
                # Tricky case: sometimes nested
                # Check for nested split folders? (As per dataset.txt content)
                # But simple case first
                print(f"No CT found for {subject}")
                continue
                
            ct_path = os.path.join(subj_raw_path, possible_cts[0])
            
            # Find Mask and JSON in derivatives
            if not os.path.exists(subj_deriv_path):
                 print(f"No derivatives found for {subject}")
                 continue
                 
            # Mask names: sub-verse004_seg-vert_msk.nii
            # JSON names: sub-verse004_seg-vb_ctd.json
            possible_masks = [f for f in os.listdir(subj_deriv_path) if 'msk' in f and (f.endswith('.nii') or f.endswith('.nii.gz'))]
            possible_jsons = [f for f in os.listdir(subj_deriv_path) if 'ctd.json' in f]
            
            if not possible_masks or not possible_jsons:
                print(f"Missing mask or json for {subject}")
                continue
                
            mask_path = os.path.join(subj_deriv_path, possible_masks[0])
            centroid_json_path = os.path.join(subj_deriv_path, possible_jsons[0])
            
            # Identify Patient ID from folder name (e.g., sub-verse012)
            patient_id = subject
            
            # Check if this patient is in our target list (vertebra_data.json)
            # vertebra_data.json keys use sub-verse012, which matches folder name
            if patient_id not in target_vertebrae:
                # Try handling splits if needed, but for now strict match
                print(f"Skipping {patient_id} (not in vertebra_data.json)")
                continue

            v_ids = target_vertebrae[patient_id]
            ...
            try:
                ...
                # fix: outputsize passed correctly
                process_mask3d(ct_path, mask_path, centroid_json_path, v_ids, output_folder, outputsize=(128,128,64))
            except Exception as e:
                print(f"Error processing {patient_id}: {e}")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Straighten Vertebrae from 3D CT Scans")
    parser.add_argument("--dataroot", type=str, required=True, help="Root directory containing dataset-verse19* folders")
    parser.add_argument("--output", type=str, required=True, help="Output directory for straightened results")
    parser.add_argument("--json", type=str, required=True, help="Path to vertebra_data.json")
    parser.add_argument("--sample", type=int, default=None, help="Number of subjects to process per dataset split (optional)")

    args = parser.parse_args()

    process_standard_verse_dataset(args.dataroot, args.output, args.json, args.sample)
